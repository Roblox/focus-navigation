...
local useLastInputMode = require(script.Parent.Parent.useLastInputMode)
...
type Props = {
	onLastInputMode: (string) -> (),
	children: any?,
}

local function TrackLastInputMode(props: Props)
	local lastInputMethod = useLastInputMode()
	props.onLastInputMode(lastInputMethod)

	return props.children
end

it("should capture the most recent input mode when first run", function()
	Rhodium.VirtualInput.Mouse.click(Vector2.new(1, 1))

	local spy = jest.fn()
	render(React.createElement(TrackLastInputMode, { onLastInputMode = spy }))

	expect(spy).toHaveBeenCalled()
	expect(spy).toHaveBeenLastCalledWith("Cursor")
end)
...
it("should trigger updates when the input method changes", function()
	...
	expect(spy).toHaveBeenLastCalledWith("Focus")
