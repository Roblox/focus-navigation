...
A library for handling gamepad UI focus management in React applications. This is intended to be the primary API surface for the focus-navigation workspace.
...
```
Re-exports the `FocusNavigationService` [from the focus-navigation package](../focus-navigation.md#focusnavigationservice).

### EngineInterface
```lua
type EngineInterface = FocusNavigation.EngineInterface
```
Re-exports the `EngineInterface` [from the focus-navigation package](../focus-navigation.md#engineinterface).
...
If a valid last-focused descendant exists when refocusing, it will be redirected to, using [`isValidFocusTarget`](../focus-navigation.md#isvalidfocustarget) to determine validity. If no valid targets are found, the default will be used.
